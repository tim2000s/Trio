...
public enum BoostV5AutoConfig {
    ...
    public struct PriorDosing: Sendable {
        ...
        public init(
            ...
            self.daysWithData = daysWithData; self.bgReadingCount = bgReadingCount
            self.tddMedianU = tddMedianU; self.manualBolusesU = manualBolusesU
            self.smbAmountsU = smbAmountsU; self.tbrBelow70Pct = tbrBelow70Pct
            self.timeBelow54Pct = timeBelow54Pct; self.meanGlucoseMgdl = meanGlucoseMgdl
            self.currentMaxIobU = currentMaxIobU; self.currentMaxBolusU = currentMaxBolusU
        }
    }
    ...
    public static func compute(_ p: PriorDosing) -> Suggestion? {
        ...
        reasons.append("HypoCaution \(hypoCaution) (TBR<70 \(pct(p.tbrBelow70Pct)), <54 \(pct(p.timeBelow54Pct)) vs targets 4%/1%)")
        ...
        reasons.append("Aggression \(aggression) (\(aggression < 1.0 ? "gentle — hypo history" : "neutral"); refines after shadow period)")
